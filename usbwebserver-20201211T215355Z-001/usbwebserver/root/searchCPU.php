...
<nav class="topnav navbar-inverse">
  <div class="container-fluid">
    ...
    <li class="active"><a href="searchCPU.php">Computer Processors</a></li>
...
$sql = "SELECT CPU_id, CPU_name, CPU_price, CPU_manufactuer, CPU_corecount, CPU_coreclock, CPU_boostclock from CPU";
$result = $mysqli->query($sql);

if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
    echo "ID: " . $row["CPU_id"]." - Name: " . $row["CPU_name"]. " - Price: " . $row["CPU_price"]. " - Manufacturer: " . $row["CPU_manufactuer"]
	.	" - Core Count: " . $row["CPU_corecount"]. " - Core Clock: " . $row["CPU_coreclock"]. " - Boost Clock: " . $row["CPU_boostclock"]. "<br>";
  }
} else {
  echo "0 results";
}
